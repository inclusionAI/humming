...
def _assert_downstream_vllm_constraints(config: dict) -> dict:
    """vLLM's _fixup_moe_tuning_config rejects K-blocks > 128 and requires
    warp_n >= 32 whenever block_n % 32 == 0, so emit compliant tiles directly."""
    block_shape = config["block_shape"]
    warp_shape = config["warp_shape"]
    assert block_shape[2] <= 128, f"K-block {block_shape[2]} exceeds vLLM limit 128"
    if block_shape[1] % 32 == 0:
        assert warp_shape[1] >= 32, f"warp_n {warp_shape[1]} below vLLM minimum 32"
    return config


class Sm90H200Heuristics(Sm90Heuristics):
    """SM90 heuristics calibrated for the H200's 132 SMs and W4A8 MoE shapes.

    Reuses the SM90 candidate-policy infrastructure from Sm90Heuristics and
    ports the H20's structural tile-selection logic with constants rescaled
    for the 132-SM (vs 78-SM) grid and the fp8-activation/int4-weight mix.
    """

    max_smem_size: int = 227 * 1024  # same die family as H20 (GH100-class SM)
    b4_allowed_dtypes: list[dtypes.DataType] = []  # int4 weights travel via b_dtype
    sm_version: int = 90
    # H200 has 132 SMs vs H20's 78; wave-occupancy targets below are derived
    # from num_sms at runtime, the ratios here only gate structural branches.
    _output_grid_dense_fraction: float = 0.6  # denser grid: 132 SMs absorb more output tiles per wave than H20's 0.5
    _stream_k_min_k_tiles: int = 48  # more SMs need a longer K to keep stream-K slices busy (H20: 64 k-tiles at 78 SMs)

    @classmethod
    def get_num_sms(cls):
        # Kept for readability at call sites; upstream DeviceHeuristics no
        # longer exposes get_num_sms, so read the live device directly.
        return current_device.sm_count
    ...
    def _get_small_m_dense_override(
        ...
        Ported from Sm90H20Heuristics; the a_bits in (8, 16) gate already
        admits fp8 activations, so the W4A8 path is exercised here."""
        ...
        if shape_m > block_shape_m or a_bits not in (8, 16) or shape_n % 128:
            return None
        # W4A8 (fp8 a, int4 b) qualifies: b is an integer type at 4 bits.
        ...
        reference_tiles = (
            math.ceil(shape_m / block_shape_m)
            * (shape_n // 128)
            * (shape_k // reference_k)
        )
        ...
        # Same accumulator budget as H20: 132 SMs do not change per-CTA
        # register/smem pressure, only grid fill.
        max_output_values = 6 * 1024
        # W4A8: a_bits=8 and b_bits=4, so wide_k stays at reference_k; the
        # 2x widening only pays off for a_bits==16 (memory-bound A) here.
        ...
        # With 132 SMs each slice must cover more tiles to fill a wave, hence
        # the / 4 quantization is kept but stage4_active_ctas scales up.
        ...
        if (
            ...
            for num_stages in range(4, 7):
                ...
                if smem_size > cls.max_smem_size * 0.8:
                    continue
                slice_iters = (
                    math.ceil(wide_num_iters / num_sms / num_stages) * num_stages
                )
                active_ctas = min(num_sms, math.ceil(wide_num_iters / slice_iters))
                # b_bits=4 for the W4A8 mix gives the full pipeline gain.
                pipeline_gain = 0.05 * min(b_bits, 4) / 4
                stage_scores.append((active_ctas * (1 + pipeline_gain * num_stages), num_stages))
            if stage_scores:
                # wide_k is 2 * (512 // a_bits); for fp8 that is 128, at the
                # vLLM K-block limit; for a_bits=16 it is 64 — both legal.
                assert wide_k <= 128
                return make_config(
        ...
        # fp8 warp_k is 64, so turns accumulate faster than H20's bf16 case;
        # the 32/64 turn thresholds carry over unchanged in tile count.
        ...
        # 132 SMs raise the n-tile budget vs H20's / 8 (dense fp8 N panels).
    ...
    def _fit_dense_block_m_to_output_grid(
        ...
        Ported from Sm90H20Heuristics with wave-occupancy fractions rescaled
        for 132 SMs (get_num_sms supplies the count at runtime)."""
        ...
        if layer_config.shape_k <= 1024:
            # Denser grid on H200: 132 SMs retire more output tiles per wave.
            if current_output_tiles >= math.ceil(cls.get_num_sms() * cls._output_grid_dense_fraction):
                return block_m
        else:
        ...
        target_output_tiles = math.ceil(
            cls.get_num_sms() * target_wave_fraction / stream_k_grid_gain
        )
        ...
        if layer_config.shape_k > 1024 and block_m >= 32:
            reuse_candidates = [
                candidate_m for candidate_m in padding_safe_candidates if candidate_m >= 16
            ]
            if reuse_candidates:
    ...
    def _tune_long_k_moe_residency(
        ...
        Ported from Sm90H20Heuristics; thresholds scale with the 132-SM grid
        so W4A8 MoE layers fill waves on H200."""
        ...
        if layer_config.shape_n >= 1024 and layer_config.shape_n % 512 == 0:
            ...
            wide_block_n = 0
        # W4A8 MoE: 64-wide K at 3 stages keeps smem per CTA low enough for
        # 2-3 resident CTAs on the 132-SM grid; K-block 64 satisfies vLLM.
        wide_block_k = 64
        wide_warp_n = 64
        wide_num_stages = 3
        wide_output_tiles = 0
        ...
        # H200's 132 SMs (vs 78) need ~1.7x the tiles for a full wave, so the
        # wide-grid threshold stays at 2 * num_sms (already SM-relative).
        ...
        # Lowered from 16 to 12: fewer wasted tail CTAs on a 132-SM grid when
        # the W4A8 K-loop is shorter than H20-calibrated shapes assumed.
        stream_k_grid_gain = min(12, max(4, wide_k_tiles // (2 * wide_num_stages)))
        stream_k_can_fill_grid = (
            layer_config.b_dtype.num_bits <= 4
            and wide_output_tiles * stream_k_grid_gain >= 3 * num_sms_physical
            and wide_k_tiles >= cls._stream_k_min_k_tiles
        )
        ...
        if use_wide_moe_tile:
            ...
            wide_num_ctas = min(
                2, cls.max_smem_size // wide_smem_size, 1024 // wide_num_threads
            )
            if wide_num_ctas >= 1:
                config.update(
                ...
                _assert_downstream_vllm_constraints(config)
                return

        num_output_tiles = estimated_m_blocks * (layer_config.shape_n // block_n)
        # H200 sweep: 2 resident CTAs beat 3 on W4A8 MoE — the third CTA
        # steals smem stages without adding usable occupancy at 132 SMs.
        num_ctas_per_sm = min(2, cls.max_smem_size // smem_size, 1024 // num_threads)
    ...
    def get_base_config(
        ...
        if a_dtype.num_bits == 16:
            ...
            # W4A8 MoE target: fp8 a, group_size 128. Cap K at 128 (vLLM
            # limit) instead of H20's 1024 // a_bits = 128 for fp8 — identical
            # here, but asserted by construction below.
            block_k = min(128, 1024 // a_dtype.num_bits)
            return {
                "block_shape": (64, 128, block_k),
                # warp_n 16 on H20 violates the vLLM warp_n >= 32 rule for
                # block_n 128 on the MoE path; use 32 on H200.
                "warp_shape": (64, 32, block_k),
                "num_ctas_per_sm": 2,
            }
        else:
            block_k = min(128, 512 // a_dtype.num_bits)
            return {
                "block_shape": (64, 128, block_k),
                "warp_shape": (64, 32, block_k),
    ...
    def get_config(
        ...
        if not layer_config.num_experts:
            ...
            # Slightly larger small-M tiles than H20: with 132 SMs, tiny
            # block_m values under-fill waves even for 256-expert MoE.
            ...
            # Large per-expert token counts: H200 sweep favors generic-SM90
            # style large-N tiles with a single resident CTA and stream-K off
            # ((64-128, 256, 128), ctas=1) — the H20-calibrated (64, 128)
            # two-CTA tile leaves ~25% throughput on the table here.
            per_expert_m = shape_m / layer_config.num_experts
            if per_expert_m >= 48 and layer_config.shape_n % 256 == 0:
                large_m_tile = True
                block_shape_m = 128 if per_expert_m >= 96 else 64
                block_shape_n = 256
                warp_shape_n = 32
                num_ctas_per_sm = 1
        ...
        if num_warps == 4:
            # Keep K-block within the vLLM 128 cap: 2 * (512 // a_bits) is
            # 128 for fp8 and 64 for 16-bit activations — both legal.
        ...
        if num_warps <= 8 and block_shape_m <= 32:
            if is_moe and warp_shape_n == 64:
                ...
                # Guard the widening against the vLLM K-block cap.
                if block_shape_k > 128:
                    block_shape_k = 128
                    num_warps_k = block_shape_k // warp_shape_k
        ...
        if warp_shape_k == block_shape_k and warp_shape_k == 512 // layer_config.a_dtype.num_bits:
            ...
            if smem_size * num_ctas_per_sm < cls.max_smem_size:
                # Only widen when the doubled K stays within the vLLM cap
                # (fp8: 64 -> 128 legal; 16-bit: 32 -> 64 legal).
                if block_shape_k * 2 <= 128:
        ...
        config = {
            ...
            "use_stream_k": not large_m_tile and layer_config.shape_k > 1024,
        ...
        if not is_moe and not use_batch_invariant:
            override = cls._get_small_m_dense_override(layer_config, shape_m, block_shape_m)
            if override is not None:
                _assert_downstream_vllm_constraints(override)
