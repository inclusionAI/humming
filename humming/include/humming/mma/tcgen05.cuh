...
CUDA_INLINE void tcgen05_transpose_bit(uint32_t (&values)[8]) {
...
struct TCGEN05 : WMMA<Ctx, ArithClass> {
  ...
  CUDA_INLINE TCGEN05(Ctx &ctx, ArithClass &arith) : Base(ctx, arith) {}

  CUDA_INLINE static void init(SharedStorage &smem) {
    if (threadIdx.x < 32) {
      tcgen05_alloc<kTmemColumns>(cast_smem_ptr_to_uint(&smem.tcgen05_tmem_col));
    }
    if (threadIdx.x == 0) __mbarrier_init(&smem.tcgen05_mbar, 1);
    __syncthreads();
  }

  CUDA_INLINE static void dealloc(SharedStorage &smem) {
    if (threadIdx.x < 32) tcgen05_dealloc<kTmemColumns>(smem.tcgen05_tmem_col);
  }
  ...
  CUDA_INLINE void transform_b(uint32_t buffer_id, uint32_t iter_id) {
    ...
    uint32_t address = ctx.smem.tcgen05_tmem_col + operand_buffer * kOperandColumns + iter_id * 8;
  ...
  CUDA_INLINE void run(uint32_t stage_id, uint32_t iter_id) {
    ...
    if (threadIdx.x == 0) {
      uint32_t base = ctx.smem.tcgen05_tmem_col;
      PRAGMA_UNROLL
      ...
      tcgen05_commit(cast_smem_ptr_to_uint(&ctx.smem.tcgen05_mbar));
    }
    first_issue = false;
    operand_buffer ^= 1;
  }
  ...
  CUDA_INLINE void wait_stage() {
    mbarrier_wait(&ctx.smem.tcgen05_mbar, phase);
    phase ^= 1;
    tcgen05_fence_after_thread_sync();
  }
  ...
  CUDA_INLINE T *final_regs_c_as_ptr() {
    ...
    for (uint32_t m = 0; m < BlockShape::M / 8; m++) {
      uint32_t values[8];
      tcgen05_ld_32x32b_x8(ctx.smem.tcgen05_tmem_col + kAccumulatorColumn + m * 8, values);
      tcgen05_wait_ld();
      ...
      tcgen05_transpose_bit<1, 4>(values);
      tcgen05_transpose_bit<2, 8>(values);
      tcgen05_transpose_bit<4, 16>(values);
      PRAGMA_UNROLL
